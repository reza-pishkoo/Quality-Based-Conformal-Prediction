...
def detect_varying_key(df, candidates):
    # pick the first candidate that has >1 unique values
    for c in candidates:
        if c in df.columns and df[c].nunique(dropna=True) > 1:
            return c
    # fall back to any numeric column that varies (excluding y metrics)
    num_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
    for c in num_cols:
        if c not in {"coverage", "avg_size", "n", "alpha"} and df[c].nunique(dropna=True) > 1:
            return c
    return None
...
def agg_stats(df, xkey):
    ...
    # standard error and 95% CI (optional)
...
def main():
    ...
    ap.add_argument("--xkey", default=None, help="Column to sweep on (auto-detect if omitted)")
    ...
    ap.add_argument("--subset-order", nargs="*", default=["easy_new","overall","hard_new"],
                    help="Order of subsets in the 2x3 grid (top-left to right, then bottom row)")
    ap.add_argument("--out", default="figure.png")
    args = ap.parse_args()

    df = pd.read_csv(args.csv)

    # Coerce key numeric columns we care about
    numeric_candidates = [
        "coverage","avg_size","n","alpha",
    ...
    # Decide xkey
    xkey = args.xkey or detect_varying_key(df, ["size_new","p_old","p_new"])
    if xkey is None:
        print("Could not detect a varying x-axis column. Pass --xkey explicitly.")
        return
    if xkey not in df.columns:
        print(f"xkey '{xkey}' not in dataframe.")
        return
    ...
    # Order methods (whatever you have in the CSV)
    ...
    label_map = {
        ...
        # keep unknowns as their original name
    }
    ...
    # Build figure: 2 rows (coverage, size) x 3 columns (easy/overall/hard)
    ...
    for j, sub in enumerate(subsets):
        ...
        ax_cov.set_title(titles.get(sub, sub))
        ax_cov.set_ylabel("Coverage")
        ax_cov.axhline(0.9, ls="--", lw=1, color="gray", alpha=0.7)
        ...
        if j == ncols - 1:
            # legend on the right-most column (top panel)
